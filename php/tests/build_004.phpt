--TEST--
ffindex_build test: multiple files + sort
--SKIPIF--
...
$files = array(dirname(__FILE__)."/build_002.phpt", dirname(__FILE__)."/build_001.phpt");
var_dump(ffindex_build($data_name, $index_name, $files, true));
var_dump( file_exists($data_name) );
var_dump( file_exists($index_name) );

exec("../src/ffindex_build -s $data_name2 $index_name2 ".implode(" ", $files));
echo "Check:\n";
